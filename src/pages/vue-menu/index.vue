  ...
  <div class="public-account-management clearfix">
    ...
    <div v-if="!showRightFlag" class="right">
      <div class="configure_page">
        ...
        <div>
          <span>菜单名称：</span>
          <a-input class="input_width" v-model="tempObj.name" placeholder="请输入菜单名称" clearable></a-input>
        </div>

        <div>
          <div class="menu_content">
            <span>菜单内容：</span>
            <a-radio-group v-model="tempObj.type">
              <a-radio :value="'media_id'">发送素材</a-radio>
              <a-radio :value="'view'">跳转链接</a-radio>
              <a-radio :value="'click'">发送关键词</a-radio>
          ...
          <div class="configur_content">
            <div class="material" v-if="tempObj.type == 'media_id'">
              ...
              ></a-input>
              <!--下面点击“选择素材”按钮，弹框框-->
              <a-popover placement="left" width="700" v-model="visible2">
                <a-table :data="tableData" style="width: 100%">
                  <a-table-column label="文件名" width="600">
                    <template slot-scope="scope">
                      <a-popover trigger="hover" placement="top">
                        <p>文件名: {{ scope.row.name }}</p>
                        <div slot="reference" class="name-wrapper">
                          <a-tag size="medium">{{ scope.row.name }}</a-tag>
                        </div>
                      </a-popover>
                    </template>
                  </a-table-column>
                  <a-table-column label="操作">
                    <template slot-scope="scope">
                      <a-button size="small" @click="handleEdit(scope.$index, scope.row)">选择</a-button>
                    </template>
                  </a-table-column>
                </a-table>

                <a-button slot="reference" type="primary">选择素材</a-button>
              </a-popover>
            ...
            <div v-if="tempObj.type == 'view'">
              <span>跳转链接：</span>
              <a-input class="input_width" v-model="tempObj.url" placeholder="请输入链接" clearable></a-input>
            </div>
            <div v-if="tempObj.type == 'click'">
              <div>
                <span>关键词：</span>
                <a-input class="input_width" v-model="tempObj.key" placeholder="请输入关键词" clearable></a-input>
              </div>
    ...
    <div style="position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%);">
      <a-button type="primary" @click="saveFun">保存并发布至菜单</a-button>
    </div>
  </div>
</template>
<script>
